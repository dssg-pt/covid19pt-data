...
curl --connect-timeout 3 -gsN 'https://covid19.min-saude.pt/numero-de-novos-casos-e-obitos-por-dia/' \
	| perl -ne 's/http/\nhttp/g;print' \
	| perl -ne '/(http[^"]+\.xls[^"]*)/&&print"$1\n"' \
	| sort -n | uniq \
	| while read a ; do 
		b=${a:0:78}
		[ "x${b:77}" = "xx" ] && b=${b:0:72}-${b:72:2}-${b:74:2}
		c=$(basename $b)
		[ -r "dados_diarios/$c.xlsx" ] && continue
		echo $a
		curl --connect-timeout 3 -o "dados_diarios/$c.xlsx" "$a"
	done 
